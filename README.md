...
**Phase 2B与Phase 2整体研究范围已完成，但策略未达到部署条件。** 已冻结2021-06-21至2026-09-02共50,863行全市场后复权行情；截至行情截止日88只已正式上市并有真实历史。月频20日动量top 20%纯多头组合净收益 **+16.39%**，同期当前名单可交易等权 **+0.83%**、932047全收益 **+3.45%**。20日rank IC仅0.0496、朴素t值1.075。交易所上市公告已重建63个月末成员，历史成员闸门通过；资产类型、全市场DPU、NAV和国债vintage仍未通过。详见[Phase 2最终结论](docs/phase2_findings.md)。

**Phase 3：跨资产经营数据库与公告事件研究已完成，但策略不部署。** 新增180201、508018两只高速及180301、508056两只物流的110条人工核验季度经营观测，并与四只能源REIT合成73条PIT同比特征。2024Q3—2026Q2共有8个至少含6只证券的横截面；top-2净收益 **+11.38%**，当期合资格证券等权 **+2.11%**、932047全收益 **+0.68%**。但平均20日rank IC仅 **0.02**、8期仅3期为正；历史成员闸门已经通过，横截面、DPU与NAV覆盖仍未通过。详见[Phase 3最终结论](docs/phase3_findings.md)。

**Phase 4：基金时点数据库与联合模型研究闭环已完成，但策略不部署。** 8只主样本新增343条基金指标、28条年末NAV、76次实际DPU和8,098行不复权行情；6份扫描公告已经逐页视觉复核。联合top-2净收益 **+11.19%**，合资格等权 **+2.11%**、932047全收益 **+0.68%**。历史成员和209/209个来源哈希已完成，但平均20日rank IC仅 **0.041**、只有8期，419条观测尚待独立复核，冻结规则后的前瞻季度仍为0。详见[Phase 4结论](docs/phase4_findings.md)。
...
**研究数据库已形成历史成员、能源、高速、物流和基金时点层。** 交易所证据确认89只已上市证券，形成63个月、2,559行月末可交易成员；其中181001于2026-09-03上市，尚晚于冻结行情截止日。2026-09-05实时源曾返回95行，交易所差集确认新增未上市508095，其7条公告已入库；因行情源随后限流，未把缺少明细的结果伪装成完整快照。Phase 4的419条观测对应209个官方URL，已全部计算SHA-256；独立复核队列仍为0/419。仓库不保存原始PDF。详见[数据库当前状态](docs/database_status.md)、[数据库维护流程](docs/database_operations.md)和[数据字典](docs/data_dictionary.md)。

**全市场分类与分派解析层已建立。** 公告目录中的95只证券均取得资产类型证据：14只为正式报告人工核验，81只为交易所公告标题明确命中，二者在状态字段中严格区分。1,626份定期报告／分派公告进入文档解析队列。495份分派公告中自动解析成功369份，并合并Phase 4已有视觉复核结果，形成378次实际DPU、覆盖67只证券的时点面板；每条事件均连接官方URL和SHA-256，并进入独立复核队列。当前126份自动失败明确保留，其中96份为深交所临时403、30份为无文字层扫描件，没有补值。
...
**全市场估值连接层已建立。** 新浪备用源冻结50,863行不复权日线，覆盖2026-08-28快照中的88只已有交易历史证券；181001在研究截止日后上市，另5只仍处发行观察阶段。与Phase 4的8,098条重叠行情逐字段一致。基金就绪度表显示95只公告观察证券中81只有基金事实，75只具备账面NAV与不复权价格连接条件，67只具备实际DPU与不复权价格连接条件；这些状态仍标记为待独立复核，不等于估值因子已经通过样本外检验。扩募／更名层另登记225份官方标题候选和10次报表间份额跳变，但公告日不冒充事件生效日。
...
Phase 4工程已完成，但`deployable=false`。规则已于2026-09-05冻结；历史成员闸门已通过。此后至少追加4个真实前瞻季度，并完成419条观测的独立复核，再决定是否进入纸面跟踪。
...
# 离线重建正式上市记录、月末成员、来源清单和独立复核队列
python -m creit_quant.phase4_database

# 仅联网补齐缺少的来源SHA-256；PDF只在内存中处理
...
- 实时行情源仍只有一个同期快照；历史成员由交易所正式上市公告回溯重建，不能把`first_observed_date`当作上市日期。今后仍需持续追加同期快照并监控终止上市事件。
- 95只公告观察对象均已有粗粒度资产类型证据，但其中81只只是交易所标题规则命中，不等于逐份报告人工核验；能源细分、混合资产和扩募后资产边界仍需单独核对。
...
- 全市场不复权估值行情已覆盖88只，P/NAV和实际DPU收益率分别有75只、67只满足基础连接条件；但3,216条基金事实和378次DPU尚未完成独立复核，不能直接升级为生产信号。
- 225份扩募／更名公告只是官方标题候选；10次份额跳变只能把变更定位在相邻报告期之间，精确生效日与扩募后资产范围仍需逐份公告确认。
