...
Phase 2已冻结50,863行后复权日线，当前94只代码中88只有历史、6只上游明确无历史。价格与成交额足以构建研究因子；但当前只有2026-08-28一个universe快照，不能据此宣称历史回测无幸存者偏差。

## 1.1 估值与分派：黄色／红色

公告中真实存在DPU、可供分配金额、基金份额和资产评估值，但仓库当前只有508026的2次可验证分派，NAV／基金份额时点面板仍为0。当前NAV不能回填历史。Phase 2因此没有计算全市场分派收益率、P/NAV或相对国债利差；这些属于数据库建设缺口，而不是被验证无效的因子。
...
Open-Meteo 可以按经纬度获取降水、温度、风速等历史数据，且再分析、拼接历史预报和固定提前期Previous Runs使用不同接口。508026已验证固定ERA5长期序列及固定GFS global提前24小时序列。

关键时点原则：再分析数据是利用事后信息重建的最佳天气估计，适合解释已实现经营结果；严格交易回测应使用固定提前期历史预报，并保存模型、有效时间、提前期及发布延迟缓冲。否则会产生前视偏差。
