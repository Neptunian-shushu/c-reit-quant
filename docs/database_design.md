...
全市场观察层已保存2026-08-28完整快照，共94只产品。2026-09-05实时源曾返回95行，交易所公告差集确认新增未上市508095；其公告已入库，但限流后无法再次取得完整明细，因此未生成伪快照。另由88条交易所上市交易提示公告和508008扫描公告书重建89只正式上市记录，形成63个月、2,559行月末可交易成员；181001于9月3日上市，不会提前进入8月成员。目录未发现终止上市候选，今后仍须持续监控并追加同期快照。Phase 4的419条观测对应209个官方URL且SHA-256齐全；全市场季报、中报和年报已统一为3,216条基金事实、覆盖81只证券，亦全部带来源哈希。机器抽取、视觉复核与独立人工核验状态严格分开。最新状态见[数据库当前状态](database_status.md)。
...
能源、两只高速和两只物流的PIT横截面已经证明版本血缘、扩募边界、公告后交易日期与市场行情可以连接。全市场不复权行情现已覆盖88只，75只具备P/NAV基础连接条件，67只具备实际DPU收益率基础连接条件；这解决了字段缺失闸门，但尚未解决独立复核和样本外有效性。下一步继续数据库优先：

1. 全市场实际DPU已形成378次／67只的首版面板；继续处理96份限流公告和30份扫描公告；
2. 季报、中报和年报已形成3,216条／81只统一基金事实；继续处理69份深交所403定期报告，并对机器抽取执行独立复核；
3. 对Phase 3的110条非能源观测执行独立二次复核，并增加PDF页码／表格定位字段；
4. 对225份扩募／更名公告候选分阶段去重，为10次报表份额跳变补齐正式生效日与资产范围；
5. 连续保存至少12个月末universe快照，并维护上市、终止与扩募状态；
6. 独立复核完成后，再在新增样本测试分派收益率、P/NAV、月频20日动量及经营surprise的联合排序。
